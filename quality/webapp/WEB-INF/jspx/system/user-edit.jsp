...
<form id="saveSysUserForm" action="${pageContext.request.contextPath}/system/user/save" method="post" 
	...
	<input type="hidden" id="sysUserRoleIds" name="roleIds" value="${jashon_str:joinEntries(u.roles, 'id', ',')}"/>
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add"><span submitForm="saveSysUserForm" beforeExecFun="checkUserDeptRole">保存用户信息</span></a></li>
		</ul>
	</div>
	<table class="edit">
		<tbody>
			...
			<tr>
				<th>真实姓名</th>
				<td><input name="u.name" value="${u.name}" class="required" maxlength="10" style="width:95%" autocomplete="off"/></td>
				<th>身份证号</th>
			...
			<tr>
				...
				<th>首页操作权限：</th><td>
					<c:forEach var="dic" items="${dics['indexPriv']}">
						<input type="radio" name="u.indexPriv" ${u.indexPriv == dic.key ? "checked" : ""} value="${dic.key}"/>${dic.value}&nbsp;&nbsp;
					</c:forEach>
				</td>
			</tr>
		</tbody>
...
<form id="sysUserRoleForm" onsubmit="return divSearch(this, 'userRoleBox');" 
		action="${pageContext.request.contextPath}/system/role/user/list/0" method="post"></form>
...
function userDeptTreeClick(event, treeId, treeNode, clickFlag){
	...
	var url='${pageContext.request.contextPath}/system/role/user/list/' + id + "?urids=${jashon_str:joinEntries(u.roles, 'id', ',')}";
