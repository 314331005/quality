...
<form id="saveSysRoleForm" action="${pageContext.request.contextPath}/system/role/save" method="post" 
	onsubmit="return validateCallback(this, afterSysRoleAjaxDone);" class="pageForm required-validate">
	<c:set var="sysRoleMenuIds" value="${jashon_str:joinEntries(r.menus, 'id', ',')}"/> 
	...
	<div class="panelBar">
		<ul class="toolBar">
			<li style="float:right;"><a class="add"><span submitForm="saveSysRoleForm" beforeExecFun="checkRoleMenu">保存角色信息</span></a></li>
		</ul>
	</div>
	...
	<table class="edit" layoutH="0">
		<tbody>
			<tr>
				...
				<td width="80%"><a class="font-blue">${d.name}</a></td>
			</tr>
			<tr>
				<th>部门编码</th>
				<td><a class="font-blue">${d.code}</a></td>
			</tr>
			<tr>
				<th>角色名称</th>
				<td>
			...
			<tr>
				...
				<td>
					<c:forEach var="dic" items="${dics['status']}">
						<input type="radio" name="r.status" ${r.status == dic.key ? "checked" : ""} value="${dic.key}"/>${dic.value}&nbsp;&nbsp;
					</c:forEach>
				</td>
			</tr>
		</tbody>
