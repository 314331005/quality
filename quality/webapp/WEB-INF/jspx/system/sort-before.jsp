...
<%@ taglib prefix="jashon_list" uri="http://www.jashon.com.cn/jstl/list"%>
...
<form id="commonSortTreeForm" name="commonSortTreeForm" action="${pageContext.request.contextPath}/system/sort/update/${className}" method="post" 
	onsubmit="return validateCallback(this, dialogAjaxDone);" class="pageForm required-validate">
<div class="panelBar">
	<ul class="toolBar">
		<li style="float:right;"><a class="add"><span submitForm="commonSortTreeForm" before="beforeSortTreeData">保存</span></a></li>
	</ul>
</div>
...
<tbody>
<c:set var="seqnos" value="${jashon_list:init()}"/>
<c:forEach var="obj" items="${objs}" varStatus="st">
	<c:set var="seqnos" value="${jashon_list:add(seqnos, obj.seqno)}"/>
	<tr id="${obj.id}">
		<td>${st.count}</td>
		<td>${obj.name}</td>
	</tr>
