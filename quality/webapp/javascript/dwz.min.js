...
(function($){
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			...
			$.ajax({
				global: op.silent == true ? false : true, /* false 不拦截 */
				type: op.type || 'GET',
				url: op.url,
				data: op.data,
				cache: false,
...
function initUI(_box){
	...
	$("form.required-validate", $p).each(function(){
		$(this).validate({
			...
			invalidHandler:function(form,validator){
				var $form=$(validator.errorContext);
				var errDetailMsg="";
				var $elem=$,text="";
				$.each(validator.invalid,function(key,value){
					$elem=$form.find(document.getElementsByName(key));
					text=$elem.attr("msg")||$elem.parent().prev().text();
					errDetailMsg='<b>'+text+'</b> 规则是: "'+value+'"';
					return false;
				});
				var errors=validator.numberOfInvalids();
				if(errors){
					var message=DWZ.msg("validateFormError",[errors,errDetailMsg]);
	...
	$("span[clearForm]",$p).each(function(){
		...
		$target.click(function(e){
			...
			$form.find("select").each(function(){
				$.fn.combox.setDefaultValue(this.id, "")
				//$(this).val(this.selectedIndex="");
			});
	...
	$("form[target=navTab]",$p).each(function(){
		var $form=$(this);
		
		var type=$form.attr("method")||"POST";
		var rel=$form.attr("rel")||"_blank";
		var title=$form.attr("title")||"";
		var fresh=eval($form.attr("fresh")||"true");
		var external=eval($form.attr("external")||"false");
		
		$form.submit(function(e){
			var url = unescape($form.attr("action")).replaceTmById($(e.target).parents(".unitBox:first"));
			if (!url.isFinishedTm()){
				alertMsg.error($form.attr("warn")||DWZ.msg("alertSelectMsg"));
				return false;
			}
			navTab.openTab(rel, url, {type:type, data:$form.serializeArray(), title:title, fresh:fresh, external:external});
	...
	$("form[target=dialog]",$p).each(function(){
		...
		var title=$form.attr("title")||"";
		var options = {};
		var w = $form.attr("width");
		var h = $form.attr("height");
		if (w) options.width = w;
		if (h) options.height = h;
		options.max = eval($form.attr("max") || "false");
		options.mask = eval($form.attr("mask") || "false");
		options.maxable = eval($form.attr("maxable") || "true");
		options.minable = eval($form.attr("minable") || "true");
		options.fresh = eval($form.attr("fresh") || "true");
		options.resizable = eval($form.attr("resizable") || "true");
		options.drawable = eval($form.attr("drawable") || "true");
		options.close = eval($form.attr("close") || "");
		options.param = $form.attr("param") || "";
	...
	$("div.dragBar",$p).each(function(){
		var $this = $(this);
		
		var resizeFor = $this.attr("resizeFor");
		var $resizeFor = $("#"+resizeFor);
		
		var initW = eval($resizeFor.width()||200);
		var minW = eval($this.attr("minW")||200);
		var maxW = eval($this.attr("maxW")||700);
		$this.css({"position":"absolute","z-index":"10","width":"5","left":initW+1,"height":"100%","float":"left","background-color":"#DFE8F6","cursor":"col-resize"});
		
		$this.drag("start",function(){
			return $this.clone().css({"opacity":.75, "background-color":"#BBCCEE"}).attr("id",this.id+"Proxy").insertAfter(this);
		}).drag(function(ev, dd){
			var left = dd.offsetX-($("#sidebar").is(":hidden") ? 35 : ($("#sidebar").width()+15));
			$(dd.proxy).css({ left: left <= minW ? minW : (left >= maxW ? maxW : left)});
		}).drag("end",function(ev, dd){
			$(dd.proxy).remove();
			var left = dd.offsetX-($("#sidebar").is(":hidden") ? 35 : ($("#sidebar").width()+15));
			var width = left <= minW ? minW : (left >= maxW ? maxW : left);
			$resizeFor.width(width);
	...
	$("div.comboxtree",$p).each(function(){
		$(this).comboxtree();
	});*/
	//custom define function//
}
...
function divSearch(form, rel){
	...
	if (rel) {
		var $box = $("#" + rel);
		$box.ajaxUrl({
			silent : eval($form.attr("silent")||false), /*是否弹出加载遮罩层*/
			...
			return this.each(function(){
				...
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger() && pageNum <= pc.numPages()) {
...
(function($){
	...
	$.fn.extend({
		...
		itemDetail: function(){
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						clickEvent: $th.attr("clickEvent")
					};
					fields.push(field);
				});
				
				function bindClickEvent($target) {
					var clickEvent = eval($target.attr("clickEvent")||null);
				...
				var addBtnId = $table.attr('addBtnId');
				if(addBtnId) {
					var $addBut = $("#" + addBtnId);
			...
			function addIndex($tbody, value) {
				var index = (parseInt($tbody.prop("index"))||0) + value;
				$tbody.prop("index", index);
			}
			
			function getIndex($tbody) {
				return $tbody.prop("index");
			}
			...
			function initSuffix($tbody) {
				...
				$tbody.find('>tr').each(function(i){
					$(':input, a.btnLook, a.btnAttach', this).each(function(){
						...
						if (val && val.indexOf("#index#") >= 0) $this.val(val.replace('#index#',index));
					});
				});
			}
			...
			function tdHtml(field){
				...
				switch(field.type){
					case 'del':
						html = '<a href="javascript:void(0)" class="btnDel" '+field.fieldClass+' clickEvent="'+field.clickEvent+'">删除</a>';
						break;
					case 'lookup':
						var suggestFrag = '';
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							+ '<input type="text" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" ' + attrFrag + '" class="'+field.fieldClass+'"/>'
							+ '&nbsp;&nbsp;<a class="font-blue btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'">' + (field.lookupText || '查找带回') + '</a>';
